...
const Freecoursepage = ({route}) => {
  ...
  console.log(fdata);
  console.log(fdata.Prc);

  return (
    
    <View style={styles.screen}>
     
     <Image style={{width:windowWidth,height:300}} source={{uri : fdata.img}}/>
      ...
      <ScrollView >
        <View style={{flexDirection:'row',alignItems:'center'}}>
          ...
          <Text style={{fontSize:30,fontFamily:'Roboto-Bold',color:'black'}}> {fdata.name} </Text>
        </View> 

        <Text style={{fontSize:22,fontFamily:'Roboto-SemiBold',color:'black',marginVertical:10}}>{fdata.desc} </Text> 

        <View style={{flexDirection:'row'}}>
          <Icon type={Icons.MaterialCommunityIcons} name="star-three-points" color="#286CF0" size={22} style={{marginTop:13,margin:5}}/>

          <Text style={{fontSize:18,fontFamily:'Nunito-Regular',color:'#0C3D9A',marginVertical:10,width:(windowWidth*85)/100,textAlign:'justify'}}>{fdata.det} </Text> 
        </View> 
        
        <View style={styles.minibox}>
            <Icon type={Icons.Ionicons} name="time-outline" color="white" size={22} style={{marginRight:5}}/>
            <Text style={{fontSize:18,fontFamily:'Roboto-SemiBold',color:'white',marginVertical:10}}>{fdata.tm} </Text> 
        </View>
